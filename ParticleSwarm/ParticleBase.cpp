...
void ParticleBase::resetAlgorithmState()
{
    m_x = 0.0;
    m_y = 0.0;
...
void ParticleBase::initializeParser(const std::string& function)
{
...
double ParticleBase::evaluateFunction(double x, double y)
{
    m_x = x;
    m_y = y;
    try {
...
double ParticleBase::partialDerivativeX(double x, double y)
{
...
double ParticleBase::partialDerivativeY(double x, double y)
{
    double x_old = m_x, y_old = m_y;
    m_x = x; // Фиксируем x
...
Result ParticleBase::validateFunctionSyntax(const std::string& function)
{
    try {
...
Result ParticleBase::checkFunctionDifferentiability(const std::string& function)
{
    try {
